...
class Game:
	def __init__(self):
		...
		self.game_active = False; self.level = Level(); self.menu_options = ['START JOURNEY', 'QUIT']; self.menu_index = 0
		
		# Introduction Logic
		self.show_tutorial = True # Restored: This must be True to see the intro
	...
	def draw_menu(self):
		self.screen.blit(self.bg_surf, (0, 0)); self.title_float += 0.05; title_y = 220 + (sin(self.title_float) * 15)
				...
				if not self.game_active:
					...
					if event.type == pygame.KEYDOWN:
						if self.show_tutorial: # Handle intro dismissal
							...
							if event.key == pygame.K_UP: self.level.shop_index = max(1, self.level.shop_index - 1)
							if event.key == pygame.K_DOWN: self.level.shop_index = min(len(weapon_data)-1, self.level.shop_index + 1)
							if event.key == pygame.K_v or event.key == pygame.K_ESCAPE: self.level.shop_active = False
							if event.key == pygame.K_SPACE:
								...
								if name not in self.level.player.unlocked_weapons and self.level.player.exp >= cost:
									self.level.player.exp -= cost; self.level.player.add_weapon(name); self.level.ui.trigger_insight(f"Bought {name.upper()}!")
								elif name in self.level.player.unlocked_weapons: self.level.ui.trigger_insight("Owned.")
								else: self.level.ui.trigger_insight("Need Scraps.")
						else:
							if event.key == pygame.K_ESCAPE: self.level.toggle_menu()
							if event.key == pygame.K_b: self.level.toggle_codex()
							if event.key == pygame.K_v: self.level.shop_active = True
